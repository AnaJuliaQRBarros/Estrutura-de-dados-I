...
// Funcao para salvar a fila em arquivo binario
...
// Nova funcao para carregar os dados do arquivo binario para a fila
void carregarFila(Fila *f, const char *nomeArquivo) {
    FILE *arquivo = fopen(nomeArquivo, "rb");
    
    // Se o arquivo nao existir, informa o usuario e retorna normalmente
    if (arquivo == NULL) {
        printf("Nenhum arquivo de dados anterior encontrado ('%s'). Iniciando com fila vazia.\n", nomeArquivo);
        return;
    }
    ...
    printf("Carregando dados salvos de '%s'...\n", nomeArquivo);
    // Le cada struct Jogador do arquivo, uma por vez
    while (fread(&jogadorLido, sizeof(Jogador), 1, arquivo) == 1) {
        // Reaproveita a funcao inserir para reconstruir a fila
        inserir(f, jogadorLido);
        contador++;
    }
    
    fclose(arquivo);
    printf("Carga concluida! %d jogador(es) restaurado(s) na fila.\n", contador);
}
...
int main() {
    ...
    // Chamada obrigatoria para carregar os dados salvos antes do menu
    carregarFila(&filaJogadores, "matchmaking_queue.bin");
